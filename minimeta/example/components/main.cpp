...
int main() {
    ...
    mmeta::typemeta_v<Math::Vec3>.dump();

    // compile-time assertions over types
    static_assert(mmeta::typemeta_v<int>.name() == "int" && "Type name should be int");
    // static_assert(mmeta::typemeta_v<Vec3>.name() == "Vec3" && "Type name should be Vec3");
    ...
    static_assert(mmeta::is_serializable_v<Math::Vec3> && "User defined types can be serialized, as long as they're annotated");
    ...
    static_assert(mmeta::classmeta_v<Math::Vec3>.version() != mmeta::classmeta_v<ColorRGB>.version() && "Binary serializer is versioned using hashes.");
